    ...
    protected function getList() {
        ...
        foreach ($results as $result) {
            ...
            $this->data['orders'][] = array(
                ...
                'total' => $this->currency->format(round($result['total']), $result['currency_code'], $result['currency_value']),
    ...
    public function invoice() {
        ...
        foreach ($orders as $order_id) {
            ...
            if ($order_info) {
                ...
                $replace = array(
                    'firstname' => ucfirst($order_info['shipping_firstname']),
                    'lastname' => ucfirst($order_info['shipping_lastname']),
                ...
                $replace = array(
                    'firstname' => ucfirst($order_info['payment_firstname']),
                    'lastname' => ucfirst($order_info['payment_lastname']),
                ...
                $this->data['isGSTAllowed'] = ($order_info['shipping_country_id'] == 99)? true: false;
                
                $payment_address = str_replace(array("\r\n", "\r", "\n"), '<br />', preg_replace(array("/\s\s+/", "/\r\r+/", "/\n\n+/"), '<br />', trim(str_replace($find, $replace, $format))));
/*echo '<pre>';
print_r($order_info); die; */              
  				$product_data = array();
  				$sample_product_id =array("4832","4833","4834","4835","4836","4837","4838","4839","4840","4841","4842","4843","4844");
                ...
                foreach ($products as $product) {  $i++;
                    ...
                    $GSTPercentTax = array();
                    $totalUnitPrice = $product['total'];
					//echo $i ."++++++++++++++++".$totalUnitPrice ."<br>";
                    ...
                    foreach ($getTaxClasses as $value) {
                        ...
                        foreach ($getTaxClassesRates as $values) {
                            if ($getTaxClasses[0]['tax_rate_id'] == $values['tax_rate_id']) {
								...
								// echo $GSTPercentTax[$value['tax_class_id']]['amount']."======================".$GSTPercentTax[$value['tax_class_id']]."<br/>";
                            }
                        }
                    }

                    $totalPrice = ($totalUnitPrice + $GSTPercentTax[$product_info['tax_class_id']]['amount']);  
                    $GSTPercentTax[$product_info['tax_class_id']]['amount'] = ($GSTPercentTax[$product_info['tax_class_id']]['amount']);

                    $tempProductTotal = $product['price'];
					
					   //echo $k."@@@@@@--".round(($tempProductTotal*0.12),2)."@@@@@@++++".round( $product['tax'],2)."<br/>";
					
					if ( round(( $tempProductTotal * 0.28),2)  == round( $product['tax'],2) ) {
                        $GSTPercentTax[$product_info['tax_class_id']]['title'] = 28;
                        $GSTPercentTax[$product_info['tax_class_id']]['rate'] = 28;
                        $GSTPercentTax[$product_info['tax_class_id']]['amount'] = $product['tax'] * $product['quantity'];
                    } elseif ( round(($tempProductTotal * 0.18),2) == round( $product['tax'],2) ) { 
                        $GSTPercentTax[$product_info['tax_class_id']]['title'] = 18;
                        $GSTPercentTax[$product_info['tax_class_id']]['rate'] = 18;
                        $GSTPercentTax[$product_info['tax_class_id']]['amount'] = $product['tax'] * $product['quantity'];
                    }elseif (round(($tempProductTotal * 0.05),2) == round( $product['tax'],2) ) {
                        $GSTPercentTax[$product_info['tax_class_id']]['title'] = 5;
                        $GSTPercentTax[$product_info['tax_class_id']]['rate'] = 5;
                        $GSTPercentTax[$product_info['tax_class_id']]['amount'] = $product['tax'] * $product['quantity'];
                    } elseif (round(($tempProductTotal * 0.12),2) == round( $product['tax'],2) ) {
						//echo  "<br/>productid=".$product['tax'];
                        $GSTPercentTax[$product_info['tax_class_id']]['title'] = 12;
                        $GSTPercentTax[$product_info['tax_class_id']]['rate'] = 12;
                        $GSTPercentTax[$product_info['tax_class_id']]['amount'] = $product['tax'] * $product['quantity'];
                    } else {
                        $GSTPercentTax[$product_info['tax_class_id']]['title'] = null;
                        $GSTPercentTax[$product_info['tax_class_id']]['rate'] = null;
                        $GSTPercentTax[$product_info['tax_class_id']]['amount'] = null;
                    }
                   /* if ((trim((double)($tempProductTotal * 28) / 100)) == trim((double)$product['tax'])) {
                        $GSTPercentTax[$product_info['tax_class_id']]['title'] = 28;
                        $GSTPercentTax[$product_info['tax_class_id']]['rate'] = 28;
                        $GSTPercentTax[$product_info['tax_class_id']]['amount'] = $product['tax'] * $product['quantity'];
                    } elseif ((trim((double)($tempProductTotal * 18) / 100)) == trim((double)$product['tax'])) {
                        $GSTPercentTax[$product_info['tax_class_id']]['title'] = 18;
                        $GSTPercentTax[$product_info['tax_class_id']]['rate'] = 18;
                        $GSTPercentTax[$product_info['tax_class_id']]['amount'] = $product['tax'] * $product['quantity'];
                    } elseif ((trim((double)($tempProductTotal * 5) / 100)) == trim((double)$product['tax'])) {
                        $GSTPercentTax[$product_info['tax_class_id']]['title'] = 5;
                        $GSTPercentTax[$product_info['tax_class_id']]['rate'] = 5;
                        $GSTPercentTax[$product_info['tax_class_id']]['amount'] = $product['tax'] * $product['quantity'];
                    } elseif ((trim((double)($tempProductTotal * 12) / 100)) == trim((double)$product['tax'])) {
						//echo  "<br/>productid=".$product['tax'];
                        $GSTPercentTax[$product_info['tax_class_id']]['title'] = 12;
                        $GSTPercentTax[$product_info['tax_class_id']]['rate'] = 12;
                        $GSTPercentTax[$product_info['tax_class_id']]['amount'] = $product['tax'] * $product['quantity'];
...
if(in_array($product['product_id'],$sample_product_id)){
	$is_sample_sale=1;
 ...
 $excl_total= $incl_total=0;
 foreach($total_data as $ototal){
	 if($ototal['code'] =='total') {
		 $incl_total=$ototal['value'];
	 }
  }
   $withot_coupon =array();
   $flag_coupon =0;
  foreach($total_data as $ototal){
 	 if($ototal['code'] =='coupon') {
		 $excl_total=$incl_total+abs($ototal['value']);
		 $flag_coupon =1;
		 $withot_coupon[] =array(
		  'order_total_id' => 0,
            'order_id' => $order_id,
            ...
            'text' => number_format($excl_total,2),
            'value' =>  $excl_total,
            'sort_order' => 7 
 ...
 if($flag_coupon==1){
	 array_splice($total_data, 2, 0,$withot_coupon);
 }
  $this->data['flag_coupon']=$flag_coupon;
 //echo '<pre>';print_r($total_data);
		...
		$customer_orders =  $this->model_sale_order->getCustomersOrdersEIS($order_info['customer_id'],$order_info['date_added']);
